...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($data['username']) && isset($data['password'])) {
        $stmt = $conn->prepare("SELECT id, id_type FROM users WHERE email=? AND password=?");
        ...
        if (!$stmt->execute()) {
            ...
            $stmt->bind_result($id, $idUserType);
            if ($stmt->fetch()) {
                // http_response_code(201);
                echo json_encode(['session_id' => 'PHPSESSID=' . session_id(), 'id_user_type' => $idUserType, 'id_user' => $id]);
            } else {
                // http_response_code(201);
        ...
        echo json_encode(['session_id' => 'PHPSESSID=' . session_id()]);
